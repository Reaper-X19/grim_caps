...
export default function Navbar() {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 pointer-events-none transition-all duration-300 ${scrolled ? 'glass shadow-lg' : 'bg-transparent'
        }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden md:flex items-center space-x-6 pointer-events-auto absolute left-1/2 transform -translate-x-1/2">
              <Link to="/configurator" className={`text-sm font-medium transition-colors ${isConfiguratorPage ? 'text-grim-accent' : 'text-white hover:text-grim-accent'
                }`}>
                Configurator
              </Link>
      ...
      {/* Auth Modal - rendered outside nav to prevent fixed positioning issues */}
